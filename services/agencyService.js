...
const validateAgencyData = async (data) => {
    ...
    if (data.officeAddress) {
        ...
        if (!street || !city || !state || !postalCode || !country) {
            throw new Error('Complete office address is required.');
        }
    }

    // Check if the business license number or agency name is already in use
...
exports.authenticateAgency = async (contactEmail, password) => {
    ...
    return agency; // Return agency object if authentication succeeds
};

// Service to validate and update agency profile
exports.updateAgencyProfile = async (userId, updatedData) => {
    ...
    if (updatedData.officeAddress) agency.officeAddress = updatedData.officeAddress;
